...
class School_model extends CI_Model
{
    ...
    public function getSchoolWithCode($code) {
        $sql = 'SELECT `si_school`.`code` AS `code` , `city_code` , `si_school`.`name` AS `school_name` , `si_city`.`name` AS `city_name` , `key` FROM `si_school` LEFT JOIN `si_city` ON `si_city`.`code` = `si_school`.`city_code` WHERE `si_school`.`code` = ?';
