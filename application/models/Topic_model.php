...
class Topic_model extends CI_Model
{
    ...
    public function getAllDynamicsByTopic($topic_id) {
        ...
        $page = $this->input->get('p'); // 页数
        $time = $this->input->get('t'); // 时间
        $state = $this->input->get('state'); // hot/time
        ...
        if (isset($page) && ($page >= 1)) {
            ...
            $query = $this->db->get('dynamic', $num_per_request, $start_index);
        } else {
            $query = $this->db->get('dynamic', $num_per_request, 0);
        }

        $dynamics = $query->result_array();
